"""
Aims:
To evaluate AMR messaging from Twitter and instagram.
To do a content analysis to understand what type of messages (themes) have been used on social media for AMR from 2017 to present.
    ...
    def get_post_info_csv(self, username):
        with open(username + '.csv', 'w', newline='', encoding='utf-8') as file:
            ...
            for post in posts:
                ...
                for comment in post.get_comments():
                    writer.writerow(
                        ["comment", comment.id, comment.owner.username, comment.text, comment.created_at_utc])
...
if __name__ == "__main__":
    ...
    cls.download_users_profile_picture("who")
    # cls.download_users_posts_with_periods("best_gadgets_2030")
    cls.download_hastag_posts("amr")
    cls.get_users_followers("who")
    cls.get_users_followings("who")
